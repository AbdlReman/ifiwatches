  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for unisex watches category
        const unisexWatchesProducts = items.filter(product =>
          product.category && product.category.includes("unisexwatches")
        );

        setProducts(unisexWatchesProducts);
        setSortedProducts(unisexWatchesProducts);
        setCurrentData(unisexWatchesProducts.slice(0, pageLimit));
      } catch (error) {
