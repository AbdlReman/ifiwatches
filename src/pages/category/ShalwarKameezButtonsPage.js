  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for Shalwar Kameez Fabric category using the category mapper
        const shalwarKameezProducts = items.filter(product => 
          hasCategory(product, "shalwarkameez")
        );
        
        setProducts(shalwarKameezProducts);
        setSortedProducts(shalwarKameezProducts);
        setCurrentData(shalwarKameezProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Shalwar Kameez Fabric", path: process.env.PUBLIC_URL + "/fashion/shalwar-kameez" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
