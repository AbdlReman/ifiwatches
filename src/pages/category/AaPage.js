...
const AaPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for aa category using the category mapper
        const aaProducts = items.filter(product => 
          hasCategory(product, "aa")
        );
        
        setProducts(aaProducts);
        setSortedProducts(aaProducts);
        setCurrentData(aaProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        titleTemplate="AA Products - IFILifestyle"
        description="Shop premium AA products at IFILifestyle. Quality, elegance, and affordability for every customer." 
      />
          ...
          <div>
            <div className="hero-banner-section">
              <Link to={process.env.PUBLIC_URL + "/shop"}>
                <img 
                  src={process.env.PUBLIC_URL + "/assets/img/banner/aa.png"}
                  alt="Shop Now"
