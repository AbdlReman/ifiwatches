...
const UnisexPerfumesPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for unisex perfumes category using the category mapper
        const unisexPerfumesProducts = items.filter(product => 
          hasCategory(product, "unisexperfumes")
        );
        
        setProducts(unisexPerfumesProducts);
        setSortedProducts(unisexPerfumesProducts);
        setCurrentData(unisexPerfumesProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
                titleTemplate="Unisex Scents - IFILifestyle"
        description="Shop versatile unisex fragrances at IFILifestyle. Perfect scents for everyone." 
      />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Unisex Scents", path: process.env.PUBLIC_URL + "/perfumes/unisex" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default UnisexPerfumesPage; 