  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for smart category
        const smartProducts = items.filter(product => product.category && product.category.includes("smart"));
        setProducts(smartProducts);
        setSortedProducts(smartProducts);
        setCurrentData(smartProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Smart Watches", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
