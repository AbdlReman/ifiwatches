  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for formal watches category using the category mapper
        const formalWatchesProducts = items.filter(product =>
          hasCategory(product, "formalwatches")
        );

        setProducts(formalWatchesProducts);
        setSortedProducts(formalWatchesProducts);
        setCurrentData(formalWatchesProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Formal Watches", path: process.env.PUBLIC_URL + "/watches/formal" },
          ]}
        />
...
export default FormalWatchesPage; 