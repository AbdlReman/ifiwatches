  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for women's watches category using the category mapper
        const womensWatchesProducts = items.filter(product => 
          hasCategory(product, "womenwatches")
        );
        
        setProducts(womensWatchesProducts);
        setSortedProducts(womensWatchesProducts);
        setCurrentData(womensWatchesProducts.slice(0, pageLimit));
      } catch (error) {
