...
const PerfumesPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const perfumesProducts = items.filter(product => 
          product.category && product.category.includes("perfumes")
        );
        
        setProducts(perfumesProducts);
        setSortedProducts(perfumesProducts);
        setCurrentData(perfumesProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
        titleTemplate="Perfumes - IFIwatches" 
        description="Shop premium perfumes at IFIwatches. Quality fragrances for every occasion and personality." 
      />
