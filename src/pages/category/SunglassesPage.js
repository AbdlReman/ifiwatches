  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for sunglasses category using the category mapper
        const sunglassesProducts = items.filter(product => 
          hasCategory(product, "sunglasses")
        );
        
        setProducts(sunglassesProducts);
        setSortedProducts(sunglassesProducts);
        setCurrentData(sunglassesProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Sunglasses", path: process.env.PUBLIC_URL + "/eyewear/sunglasses" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
