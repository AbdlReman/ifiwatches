  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for men's perfumes category using the category mapper
        const mensPerfumesProducts = items.filter(product => 
          hasCategory(product, "mensperfumes")
        );
        
        setProducts(mensPerfumesProducts);
        setSortedProducts(mensPerfumesProducts);
        setCurrentData(mensPerfumesProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Men's Fragrances", path: process.env.PUBLIC_URL + "/perfumes/mens" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default MensPerfumesPage; 