  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for luxury category
        const luxuryProducts = items.filter(product => product.category && product.category.includes("luxury"));
        setProducts(luxuryProducts);
        setSortedProducts(luxuryProducts);
        setCurrentData(luxuryProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Luxury Watches", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
