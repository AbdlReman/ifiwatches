  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for used mobiles category using the category mapper
        const usedMobilesProducts = items.filter(product => 
          hasCategory(product, "usedmobiles")
        );
        
        setProducts(usedMobilesProducts);
        setSortedProducts(usedMobilesProducts);
        setCurrentData(usedMobilesProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Trusted Used Mobiles", path: process.env.PUBLIC_URL + "/mobile-gadgets/used-mobiles" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default UsedMobilesPage; 