...
const EyewearPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for eyewear category
        const eyewearProducts = items.filter(product => 
          product.category && product.category.includes("eyewear")
        );
        
        setProducts(eyewearProducts);
        setSortedProducts(eyewearProducts);
        setCurrentData(eyewearProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO 
                titleTemplate="Eyewear - IFILifestyle"
        description="Shop premium eyewear at IFILifestyle. Quality sunglasses and optical frames for every style." 
      />
      <LayoutOne headerTop="visible">
        ...
        <div className="shop-area pt-95 pb-100">
          <div className="container">
            <ChildCategoryButtons parentCategory="eyewear" />
            <div className="row">
