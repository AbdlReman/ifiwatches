...
const SportsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const sportsProducts = items.filter(product => product.category && product.category.includes("sports"));
        setProducts(sportsProducts);
        setSortedProducts(sportsProducts);
        setCurrentData(sportsProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      <SEO titleTemplate="Sports Watches - IFILifestyle" description="Shop premium sports watches at IFILifestyle. Quality, durability, and style for active lifestyles." />
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            { label: "Home", path: process.env.PUBLIC_URL + "/" },
            { label: "Sports Watches", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
