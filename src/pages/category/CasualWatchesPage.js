  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for casual watches category
        const casualWatchesProducts = items.filter(product =>
          product.category && product.category.includes("casualwatches")
        );

        setProducts(casualWatchesProducts);
        setSortedProducts(casualWatchesProducts);
        setCurrentData(casualWatchesProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Casual Watches", path: process.env.PUBLIC_URL + "/watches/casual" },
          ]}
        />
...
export default CasualWatchesPage; 