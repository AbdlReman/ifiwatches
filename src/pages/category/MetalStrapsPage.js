  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for metal straps category using the category mapper
        const metalStrapsProducts = items.filter(product => 
          hasCategory(product, "metalstraps")
        );
        
        setProducts(metalStrapsProducts);
        setSortedProducts(metalStrapsProducts);
        setCurrentData(metalStrapsProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Metal/Chain Straps", path: process.env.PUBLIC_URL + "/watch-straps/metal" },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
...
export default MetalStrapsPage; 