  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const classicProducts = items.filter(product => product.category && product.category.includes("classic"));
        setProducts(classicProducts);
        setSortedProducts(classicProducts);
        setCurrentData(classicProducts.slice(0, pageLimit));
      } catch (error) {
      ...
      <LayoutOne headerTop="visible">
        <Breadcrumb
          pages={[
            ...
            { label: "Classic Watches", path: process.env.PUBLIC_URL + pathname },
          ]}
        />
        <div className="shop-area pt-95 pb-100">
          <div className="container">
