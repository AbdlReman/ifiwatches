  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter for fashion category using the category mapper
        const fashionProducts = items.filter(product => 
          hasCategory(product, "fashion")
        );
        
        setProducts(fashionProducts);
        setSortedProducts(fashionProducts);
        setCurrentData(fashionProducts.slice(0, pageLimit));
      } catch (error) {
  ...
  return (
    <Fragment>
      ...
      <LayoutOne headerTop="visible">
        ...
        <div className="shop-area pt-95 pb-100">
          <div className="container">
            <ChildCategoryButtons parentCategory="fashion" />
            <div className="row">
